...
# Store the models in a named list for clean extraction
model_list <- list(
...
## Extract CPO failures for all models
...
## Dunn-Smith Quantile Residuals and Q-Q Plot (FIT / CALIBRATION)
...
plot_qq_all <- ggplot(diagnostic_df, aes(sample = Residuals)) +
  ...
  labs(
    ...
    y = "PIT-based Quantile Residuals"
  ) +
...
ggsave(filename = "plot_qq_all.png", plot = plot_qq_all, width = 9, height = 6, dpi = 400, bg = "white")
...
ggsave(filename = "plot_homo_all.png", plot = plot_homo_all, width = 9, height = 6, dpi = 400, bg = "white")

# Calculate Empirical Coverage (The Dispersion Check)
...
### 3.1.3. SENSITIVITY ANALYSIS ON FIXED EFFECTS ----

library(dplyr)
library(tidyr)
library(tibble)

# Define the specific models for the sensitivity analysis (Models 2, 3, 4, 5)
sensitivity_models <- list(
  "Model 2 (Type II)" = model2,
  "Model 3 (Type III)" = model3,
  "Model 4 (Type IV)" = model4,
  "Model 5 (Bernardinelli)" = model5
)

sensitivity_df <- data.frame()

# Loop through to extract the fixed effects
for (m_name in names(sensitivity_models)) {
  m_obj <- sensitivity_models[[m_name]]
  
  fixed_raw <- as.data.frame(m_obj$summary.fixed) %>%
    ...
    mutate(
      Estimate = sprintf("%.3f [%.3f, %.3f]", mean, `0.025quant`, `0.975quant`),
      Model = m_name
    ) %>%
    select(Covariate, Model, Estimate)
  
  sensitivity_df <- bind_rows(sensitivity_df, fixed_raw)
}

# Pivot the table so Models are columns and Covariates are rows
sensitivity_table <- sensitivity_df %>%
  pivot_wider(names_from = Model, values_from = Estimate)

print("Sensitivity Analysis of Fixed Effects Across Models:")
...
# A. SPATIAL AUTOCORRELATION: Year-by-Year Moran's I 
...
# B. TEMPORAL AUTOCORRELATION: Lag-1 Correlation 
...
ggsave(filename = "plot_time.png", plot = plot_time, width = 9, height = 6, dpi = 400, bg = "white")
...
ggsave(filename = "plot_pred.png", plot = plot_pred, width = 9, height = 6, dpi = 400, bg = "white")

### E. MAP THE SPATIAL EFFECT (BYM2)

# Extract the spatial effect from Model 4
...
# Create the 'map_spatial_effect' object by joining it to your polygons
...
ggsave(filename = "plot_spatial.png", plot = plot_spatial, width = 9, height = 6, dpi = 400, bg = "white")
...
### H. MAPPING SPACE-TIME INTERACTIONS (MODEL 4: TYPE IV) ----

# Extract the raw INLA interaction matrix for Model 4
int_raw <- as.data.frame(model4$summary.random$id_space_int)

# Reconstruct the Kronecker structure (36 years x 107 provinces)
int_raw <- int_raw %>%
  mutate(
...
# Join with observed election years
...
# Join back to the spatial polygons
map_interactions <- map_prov_2022 %>%
  mutate(id_space_main = row_number()) %>%
  left_join(interaction_df, by = "id_space_main")

# Generate the Plot (Continuous Red-White-Blue Scale at the bottom)
plot_int <- ggplot(map_interactions) +
  ...
  scale_fill_gradient2(
    ...
    high = "#4393C3",      
    midpoint = 0,
    name = "Local Shock (Log-Odds)",
    # Format the color bar for a bottom placement
...
ggsave(filename = "plot_int.png", plot = plot_int, width = 9, height = 6, dpi = 400, bg = "white")

### I. STATISTICAL OUTLIER ANALYSIS (BASED ON PIT RESIDUALS) ----

# Extract PIT values from Model 4 and convert to Quantile Residuals
pit_values <- model4$cpo$pit

# Add a tiny tolerance to prevent Inf/-Inf errors if a PIT value is exactly 0 or 1
tol <- 1e-6
pit_safe <- pmax(pmin(pit_values, 1 - tol), tol)
quantile_residuals <- qnorm(pit_safe)

# Attach the residuals and fitted values to the main continuous dataset
df_outliers <- df_continuous %>%
  mutate(
    Observed = Y_turnout, # Using the exact response variable name from df_continuous
    Fitted = model4$summary.fitted.values$mean[1:nrow(df_continuous)],
    Quantile_Residual = quantile_residuals
  )

# Extract the Top 10 Positive Statistical Outliers (Unexpectedly High Turnout)
top_positive_outliers <- df_outliers %>%
  arrange(desc(Quantile_Residual)) %>%
  select(COD_PROV_22, ANNO, Observed, Fitted, Quantile_Residual) %>%
  head(10)

# Extract the Top 10 Negative Statistical Outliers (Unexpectedly Low Turnout)
top_negative_outliers <- df_outliers %>%
  arrange(Quantile_Residual) %>%
  select(COD_PROV_22, ANNO, Observed, Fitted, Quantile_Residual) %>%
  head(10)

print("Top 10 Positive Statistical Outliers (Unexpectedly High Turnout):")
print(top_positive_outliers %>% mutate_if(is.numeric, round, 4))

print("Top 10 Negative Statistical Outliers (Unexpectedly Low Turnout):")
print(top_negative_outliers %>% mutate_if(is.numeric, round, 4))
